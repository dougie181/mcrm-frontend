...
import 'dayjs/locale/en-au'; // Import the 'en-au' locale
...
const CampaignTaskForm = ({ mode, taskData, onSubmit, onClose }) => {
  ...
  const handleSaveOrUpdate = async () => {
    if (validateForm()) {
      const updatedTaskData = {
        ...localTaskData,
        due_date: localTaskData.due_date ? dayjs(localTaskData.due_date).utc().toISOString() : null,
      };
      onSubmit(updatedTaskData);  // Send the data back to the parent
    }
  };
            ...
            <>
              {isSystemGenerated && localTaskData.client_id && localTaskData.data.client_name && (
                <Typography mb={1} variant="body2" color="textSecondary">
                  Client Name: <strong>{localTaskData.data.client_name}</strong>
                </Typography>
              )}
              {/* Add client response above notes if response_type is 'auto' */}
              {localTaskData.data.response_type === "automatic" && (
                <>
                  <Typography mb={1} variant="body2" color="textSecondary">
                    Client Response: <strong>{localTaskData.data.response || "N/A"}</strong>
                  </Typography>
                  {localTaskData.data.response_timestamp && (
                    <Typography mb={1} variant="body2" color="textSecondary">
                      Responded on:{" "}<strong>
                      {dayjs
                        .utc(localTaskData.data.response_timestamp)
                        .local()
